#Code by Ash Hathaway; url: https://blog.stitchdata.com/tapping-into-the-marvel-api-b895d54e7ac4

#run python3 marvel.py -c config.json | target-csv
...
#Now let's get Singer going
  ...
  singer.write_schema('series', schema, 'id')

#call the api and get records until there aren't anymore
  while True:
  	response = requests.get('https://gateway.marvel.com:443/v1/public/series?orderBy=modified&apikey={}&ts={}&hash={}&limit={}&offset={}'.format(CONFIG['public_key'],t,hash,marvel_limit,offset))

  	body = response.json()['data']

  	singer.write_records('series', body['results'])		
